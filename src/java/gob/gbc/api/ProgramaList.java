 ...
 * @author ealonso
 */
public class ProgramaList  extends HttpServlet{
	...
	private Gson _gson;
		...
		response.setContentType("application/json;charset=UTF-8");	
                ...
                try {
                    ...
                    Logger.getLogger(ProgramaList.class.getName()).log(Level.SEVERE, null, ex);
		...
		try {
			...
			result.setTotal(rs.getProgramaListCount(session, ramo, programa, options, true));
						
                        if(options.getTake() == 0)
                            options.setTake(result.getTotal());
                        
			// set the data
			result.setData(rs.getProgramaListFiltros(session, ramo, programa, options, false)); 
