<%-- 
    Document   : ajaxGetInfoAmpliacionReduccionAccion
    Created on : Jan 05, 2016, 12:00:34 AM
    Author     : Jarguelles
--%>
...
<%
    ...
    MovimientosAmpliacionReduccion ampliacionReduccion = null;
    ...
    List<AmpliacionReduccionAccion> movtoAccionesList = new ArrayList<AmpliacionReduccionAccion>();
    ...
    try {
        ...
        if (request.getParameter("estatus") != null && !request.getParameter("estatus").equals("")) {
            ...
            if ((!status.equalsIgnoreCase("X")) && (!status.equalsIgnoreCase("R")) && (!status.equalsIgnoreCase("K")) && ((status.equalsIgnoreCase("K")) && autorizacion)) {
        ...
        if (session.getAttribute("ampliacionReduccion") != null && session.getAttribute("ampliacionReduccion") != "") {
            ...
            movtoAccionesList = ampliacionReduccion.getAmpReducAccionList();
            movtoMetasList = ampliacionReduccion.getAmpReducMetaList();
            if (identificador != -1) {
                for (AmpliacionReduccionAccion movtoAccionListTemp : movtoAccionesList) {
        ...
        if (ampliacionReduccion != null) {
            for (AmpliacionReduccionMeta meta : ampliacionReduccion.getAmpReducMetaList()) {
        ...
        if (metaHabilitada) {
            ...
            if (inputAllDisable.equals("")) {
                strResultado += "        <input type='button' value='Guardar' onclick=\"validarGuardarInfoAmpliacionReduccionAccion();\" /> ";
            ...
            if (objParametros.getValidaTodosTrimestre().equals("S")) {
                ...
                Date date = accionBean.getResultSQLgetServerDate();
            ...
            strResultado += "<center>";
            if (inputAllDisable.equals("")) {
                strResultado += "        <input type='button' value='Guardar' onclick=\"actualizarAmpliacionReduccionAccion();\" /> ";
                strResultado += "        <input type='button' value='Cancelar' onclick=\"fadeOutPopUp('infoAccion'); fadeOutPopUp('estimacionMeta');\" /> ";
            } else {
