<%-- 
    Document   : ajaxGetInfoAmpliacionReduccionRecalendarizacionAccion
    Created on : Jan 12, 2016, 11:30:34 AM
    Author     : jarguelles
--%>
...
<%
    ...
    try {
        ...
        if (movtoOficioAccion != null) {
            strResultado += "				<textarea maxlength='700' id='txtAreaAccion' class='no-enter' style='text-transform: uppercase;'  " + inputAllDisable + " >" + movtoOficioAccion.getAccionDescr() + "</textarea> ";
        } else {
            strResultado += "				<textarea maxlength='700' id='txtAreaAccion' class='no-enter' style='text-transform: uppercase;'  " + inputAllDisable + " >" + accionAct.getAccion() + "</textarea> ";
        }
        strResultado += "			</div>  ";
        strResultado += "		</td>  ";
        strResultado += "	</tr> ";
        ...
        if (objParametros.getValidaTodosTrimestre().equals("S")) {
            ...
            meses = ampliacionReduccionBean.getMesesTrimestreByMes(monthAct, validaTrimestre);
            if (!validaTrimestre) {
                if (meses.length > 0) {
                    monthAct = meses[0];
                } else {
        ...
        if (inputAllDisable.equals("")) {
            strResultado += "        <input type='button' value='Guardar' onclick='actualizarAmpliacionReduccionRecalendarizacionAccion()'      /> ";
