<%-- 
    Document   : ajaxGetInfoAmpliacionReduccionMeta
    Created on : Jan 07, 2016, 03:21:34 PM
    Author     : mavalle
--%>
...
<%
    ...
    List<AmpliacionReduccionMeta> movtoMetasList = null;
    ...
    try {
        ...
        if (session.getAttribute("ampliacionReduccion") != null && session.getAttribute("ampliacionReduccion") != "") {
            ...
            if (identificador != -1) {
                for (AmpliacionReduccionMeta movtoMetasListTemp : movtoMetasList) {
                    if (movtoMetasListTemp.getIdentificador() == identificador) {
                        movtoOficioMeta = movtoMetasListTemp.getMovOficioMeta();
                        movtoEstimacionMetaList = movtoMetasListTemp.getMovOficioEstimacion();
                    }
                }
        ...
        strResultado += "   <center>";
        if (inputAllDisable.equals("")) {
            strResultado += "        <input type='button' value='Guardar' onclick=\"actualizarAmpliacionReduccionMeta();\"/> ";
            strResultado += "        <input type='button' value='Cancelar' onclick=\"fadeOutPopUp('infoMetaAmpRed'); fadeOutPopUp('estimacionMetaAR');\"     /> ";
        } else {
