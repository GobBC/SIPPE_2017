<%-- 
    Document   : ajaxGetInfoReprogramacionAccion
    Created on : May 18, 2016, 12:00:34 AM
    Author     : jarguelles
--%>
...
<%
    ...
    List<ReprogramacionAccion> movtoAccionesList = null;
    ...
    try {
            ...
            if (identificador != -1) {
                for (ReprogramacionAccion movtoAccionListTemp : movtoAccionesList) {
                    if (movtoAccionListTemp.getIdentificador() == identificador) {
                        movtoEstimacionAccionList = movtoAccionListTemp.getMovAcionEstimacionList();
        ...
        if (metaAct.getMeta() == null) {
            ...
            for (ReprogramacionMeta movtoMetaListTemp : movtoMetasList) {
                ...
                movtoOficioMetaTemp = movtoMetaListTemp.getMetaInfo();
                ...
                if ((ramoId.equalsIgnoreCase(movtoOficioMetaTemp.getRamoId())) && (metaId == movtoOficioMetaTemp.getMetaId()) && (year == movtoOficioMetaTemp.getYear())) {
                    ...
                    movtoEstimacionMetaListTemp = movtoMetaListTemp.getMovEstimacionList();
        ...
        strResultado += "       <td> <textArea class='no-enter' id='txtAreaAccion' maxlength='300' style='text-transform:uppercase;' >" + movtoOficioAccion.getAccionDescr() + "</textArea></td>";
        strResultado += "   </tr>";
        strResultado += "</table>";
        ...
        strResultado += "<center>";
        if (inputAllDisable.equals("")) {
            strResultado += "        <input type='button' value='Guardar' onclick=\"actualizarReprogramacionAccion();\" /> ";
            strResultado += "        <input type='button' value='Cancelar' onclick=\"fadeOutPopUp('infoAccion'); fadeOutPopUp('estimacionMeta');\" /> ";
        } else {
